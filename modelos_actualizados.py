...
def comparar_modelos(roc_auc_cnn, fpr_cnn, tpr_cnn,
                      ...
                      roc_auc_if, fpr_if, tpr_if, roc_auc_reg, fpr_reg, tpr_reg):
    """
...
def build_cnn_regresion(input_shape, num_conv_layers, num_filters,
    ...
    OBJ: Construye la arquitectura de la CNN de regresión.
         ...
         la salida (lineal en vez de sigmoide) y la función de pérdida (MAE).
    PARAMS:
...
def preparar_datos_cnn_regresion(X_light, X_dark, test_size_val=0.2, random_state=42):
    """
    OBJ: Calcula el target (V_min), normaliza trazas y target (con
         parámetros calculados solo a partir de light) y separa
         train/val (solo light, igual criterio que el autoencoder).
    PARAMS:
...
def evaluar_cnn_regresion(resultados_reg):
    """
    OBJ: Evalúa la CNN de regresión: curva de aprendizaje, distribución
